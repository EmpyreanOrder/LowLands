	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float3 unpack9 = UnpackNormalScale( tex2D( _DetailNormals, uv_DetailNormals ), tex2DNode7.a );
				unpack9.z = lerp( 1, unpack9.z, saturate(tex2DNode7.a) );
				

				float3 BaseColor = ( ( saturate( 2.0f*blendOpDest14*blendOpSrc14 + blendOpDest14*blendOpDest14*(1.0f - 2.0f*blendOpSrc14) )).rgb * ( ( ( tex2D( _DetailColor, uv_DetailColor ).rgb * (unity_ColorSpaceDouble).rgb ) * temp_output_9_0_g1 ) + appendResult16_g1 ) );
				float3 Normal = BlendNormal( UnpackNormalScale( tex2D( _NormalMap, uv_NormalMap ), 1.0f ) , unpack9 );
				float3 Emission = 0;
				float3 Specular = 0.5;
				float Metallic = 0;
				float Smoothness = ( tex2DNode1.a * _LocalSmooth );
				float Occlusion = saturate( ( tex2DNode7.r + _OcclusionMult ) );
				float Alpha = 1;
...
WireConnection;20;4;2;0
WireConnection;20;5;13;0
ASEEND*/
//CHKSM=46342B37C8147FE8CF9D061DDB7AF7D0DF280161