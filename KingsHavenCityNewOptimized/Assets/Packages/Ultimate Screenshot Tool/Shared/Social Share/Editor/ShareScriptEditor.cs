        ...
        void SelectFileSettings()
        {
            ...
            if (showSelectFileSettings)
            {
                ...
                if (hasFileToShare && isGif)
                {
                    ...
                    string convertToMP4Tooltip = "Convert to MP4 using ffmpeg. Can only be used if the media to upload path is set with an existing gif.";
                    GUIContent convertToMP4GUIContent = new GUIContent("Convert to MP4", convertToMP4Tooltip);
                    if (isGif && GUILayout.Button(convertToMP4GUIContent, GUILayout.MinHeight(40)))
